...
/**
 *A iterative version of a ternary search algorithm this is better way to implement the ternary search
 *search, because a recursive version adds some overhead to a stack. but in java the compile can 
 *transform the recursive version to iterative implicitly, so there are no much differences between these two algorithm
 * 
 * 
 * <p> Worst-case performance O(log3(n)) Best-case performance O(1) Average performance O(log3(n))
