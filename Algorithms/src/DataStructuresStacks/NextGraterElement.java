 ...
 * for a given element x of array, the next Grater element of that element is the 
 ...
 *  
 *  1. if the stack is empty push in the stack 
 *  2. if the stack is not empty 
 *  
 *  	a. compare the top element of the stack with next.
 *  	b. if next is greater that the top element, pop element from the stack.
 *  		next is the necxt greater element for the popped element
 *  	c. Keep popping from the stack while the popped element is smaller than the next 
 *  	   next gretare element for all such 
 *  	   popped elements.
 *  	d. Finally, push the next in the stack.
 *  3. if the elements are left in the stack after completing while loop then their next greater element is -1.
 *
 */
